...
\SetBgContents{\scriptsize \rotatebox[origin=c]{90}{\texttt{miguelstorani @ GitHub bit.ly/2Kiclyi}}}
\SetBgScale{1}
\SetBgAngle{0}
\SetBgOpacity{1}
\SetBgColor{black}
...
{ \large \bfseries TP1A - Corrección}\\[0.7cm] % Title of your document
\HRule \\[1.5cm]
...
\includegraphics[width=5cm]{./logos/logo_utn.png}\\[1cm] 				% Include a department/university logo - this will require the graphicx package
 {\large 7 de septiembre de 2019}											% Date, change the \today to a set date if you want to be precise
%-----------------------------------------------------------------------
...
Considerando que toda función puede ser separada en su parte par y su parte impar, nuestra transformada puede reescribirse a partir de esta descomposición. Teniendo en cuenta:

$$f_{(t)} = fp_{(t)} + fi_{(t)}$$
 donde $fp_{(t)}$ es la parte par de la función y $fi_{(t)}$ es la parte impar.\\
 
 
Aplicando la transformada a ambos miembros sabiendo que la transformada de una suma es igual a la suma de las transformadas (como se expresa en la expresión \ref{linealidad} que se encuentra en la página \pageref{linealidad}) nos queda:

$$\overline{T}_{ms}(f_{(t)}) = \overline{T}_{ms}(fp_{(t)}) + \overline{T}_{ms}(fi_{(t)})$$

Se puede demostrar que la primer componente de la transformada de una función impar será nula. Se puede demostrar también que la segunda componente de una función par será nula. Teniendo esto en cuenta, podemos expresar nuestra transformada de la siguiete manera:
{\footnotesize
\begin{equation}
\overline{T}_{ms}(f_{(t)}) = \frac12
\begin{pmatrix}
\mathcal{L}\left\{fp(t)u(t)\right\}_{(z)} +  \mathcal{L}\left\{fp(t)u(-t)\right\}_{(-z)}\\[0.2 cm]
0 
\end{pmatrix} +  \frac12
\begin{pmatrix}
0\\[0.2 cm]
\mathcal{L}\left\{fi(t)u(t)\right\}_{(z)} -  \mathcal{L}\left\{fi(t)u(-t)\right\}_{(-z)} 
\end{pmatrix}
\end{equation}
}

Por lo tanto, teniendo la descomposición de la función en sus partes par e impar podemos calcular la transformada de la siguiente manera:
...
\mathcal{L}\left\{fi(t)u(t)\right\}_{(z)} -  \mathcal{L}\left\{fi(t)u(-t)\right\}_{(-z)} 
\end{pmatrix}
\label{transformada_descompuesta}
\end{equation}
...
Así como lo desarrollamos en la sección 1, se puede expresar a la transformada descomponiendo la función en estudio en sus partes par e impar (ver ecuación \ref{transformada_descompuesta}).

A partir de esta expresión la antitransformada se puede expresar como:
\begin{equation}
f_{(t)} = \frac12 \mathcal{L}^{-1}\left\{T_x\right\} + \frac12 \mathcal{L}^{-1}\left\{T_y\right\}
\label{antitransformada_descompuesta}
\end{equation}

Donde el primer término dará la parte impar de la función y el segundo término nos dará la parte par de la función.
...
\subsubsection[Solución a los sistemas para $\ f_{(x)}=\sen(x)$]{Solución a los sistemas para $\bm{ f_{(x)}=\sen(x)}$}
...
\subsubsection[Solución a los sistemas para $ f_{(x)}=\cos(x)$]{Solución a los sistemas para $\bm{ f_{(x)}=\cos(x)}$}
...
\subsubsection[Solución a los sistemas para $ f_{(x)}=\sen(x+3)$]{Solución a los sistemas para $\bm{ f_{(x)}=\sen(x+3)}$}
